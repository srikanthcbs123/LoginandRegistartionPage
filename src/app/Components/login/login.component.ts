...
export class LoginComponent implements OnInit {
  constructor(private readonly  userservice:UserService,private readonly router:Router) //DepencyInjection
  {
  ...
  (res: any) => {//THIS IS ONE SUCCESS FUNCTION.HERE WE ARE PROCESSING THE SUCCESS CODE
  ...
  (err) => {//THIS IS ONE ERROR FUNCTION.HERE IF ANY ERROR OCCURED/ERROR RELATED CODE WE ARE PROCESSING
    if (err.status == 400) {
      alert(
        "Incorrect username or password.&Authentication failed"
      );
      this.router.navigateByUrl("/home");
